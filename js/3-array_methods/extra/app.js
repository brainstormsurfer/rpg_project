...
const findHighestLevelCharacter = (characters) => {
  ...
  for (const chr of characters) {
    if (highest === chr.level) {
      ...
      highest = chr.level;
      while (highestLvlChr.highestLvlCharArr.length > 0) {
        highestLvlChr.highestLvlCharArr.shift();
      }
      highestLvlChr.highestLvlCharArr.push(chr);
    }
  }
...
const filterCharactersByLevelRange = (characters, lowBound, hiBound) => {
  return characters.filter(
    ({ level }) => level >= lowBound && level <= hiBound
  );
};
...
const findCharacterWithMostSkills = (characters) => {
  ...
  for (const chr of characters) {
    if (
      !mostSkillsObj.skills ||
      mostSkillsObj.skills.length < chr.skills.length
    )
      mostSkillsObj = chr;
  }
