...
const players = [
    {id: '45f3e', name: "Eddie Vedder",  health: 100,  level: 1,  location: "forest",  inventory: ["sword", "health p100"]},
    {id: '45g3e', name: "Axl Rose",  health: 100,  level: 2,  location: "mountain",  inventory: ["rose", "ego potion"]},
    {id: '45h3e', name: "Kurt Cobain",  health: 100,  level: 3,  location: "abyss",  inventory: ["gun", "eternity potion"]},
    {id: '45j3e', name: "Eric Clapton",  health: 100,  level: 2,  location: "desert",  inventory: ["guitar", "magic-fingers potion"]},
    {id: '45k3e', name: "Bob Dylan",  health: 100,  level: 1,  location: "boneyard",  inventory: ["pen", "non-ego potion"]},
];
...
const processEnemies = (enemies, callback) => {
    ...
    const newEnemiesFromCallBack = newEnemies.map((enemy) => callback(enemy))
    console.log('newEnemiesFromCallBack', newEnemiesFromCallBack);
...
const processQuests = (quests, callback) => {
    ...
    const newQuestsFromCallBack = newQuests.map((quest) => callback(quest))
    console.log('newQuestsFromCallBack', newQuestsFromCallBack);
...
const processPlayers = (players, callback) => {
    ...
    const newPlayersFromCallBack = newPlayers.map((player) => callback(player))
...
processPlayers(players, addHealth)

processQuests(quests, addExperience)

processEnemies(enemies, addHealth)